...
for line in sys.stdin: # read input from STDIN
  ...
  if current_email_domain == email_domain: #this works because mapper output is sorted by key (email_domain)
    #by Hadoop before feeding to reducer input
    current_count += count #if email domain hasn't changed continue counting its occurances
  else:
    if current_email_domain: #if email domain has changed output count for previous email domain to STDOUT
      # and start counting for new email domain
